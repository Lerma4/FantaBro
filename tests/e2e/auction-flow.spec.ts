...
import type { AuctionChangePayload } from '../../server/utils/events'
...
/**
 * Senza database i passi si dichiarano saltati **uno per uno**. Con `describe.skipIf` su un
 * callback `async` vitest non esegue il collector, quindi il file non registra nessun test
 * e `vitest` esce in errore per "no tests": il gate si romperebbe per chi non ha un
 * PostgreSQL a portata di mano.
 */
const step = it.skipIf(!hasDatabase)
...
/**
 * `DATABASE_URL` impostata non basta: il database deve rispondere **adesso**. E la causa
 * piu probabile di una `setup()` fallita, e merita un messaggio proprio.
 */
async function requireReachableDatabase(): Promise<void> {
  ...
  try {
    ...
    throw new Error(
      'DATABASE_URL impostata ma il database non risponde. Con Docker Desktop il port proxy ' +
        'puo incartarsi per qualche secondo: `docker exec <container> psql` continuerebbe a ' +
        'funzionare perche passa dal socket interno, quindi il container sembra sano. ' +
...
describe('flusso d asta end to end', async () => {
  ...
  //
  // Se rigetta qui, vitest smette di registrare le `it()` che seguono: la suite esce
  // "skipped" con codice 0, cioe un verde che non ha verificato niente. L'errore va
  // catturato e asserito, che e l'unico esito onesto quando il database c'era.
  let setupError: unknown
  if (hasDatabase) {
    try {
      await requireReachableDatabase()
      await setup({ server: true, buildDir: E2E_BUILD_DIR })
    } catch (error) {
      setupError = error
    }
  }

  step('0. il database risponde e il server di test si e avviato', () => {
  ...
  //
  // L'errore va catturato, non lanciato: in vitest un `beforeAll` che rigetta marca i test
  // della suite come **skipped**, quindi anche il passo 0 - il solo che racconterebbe cosa
  // e andato storto - verrebbe saltato, e la suite uscirebbe verde senza aver provato niente.
  beforeAll(() => {
    if (!hasDatabase || setupError) return
    try {
      // Comando costante, nessun input interpolato: non c'e superficie di injection. La
      // shell serve perche su Windows `pnpm` e uno script `.cmd`, che `execFileSync` non
      // sa avviare - lo stesso motivo per cui i provider AI risolvono il percorso a mano.
  ...
  step('1. rifiuta una route d asta senza sessione', async () => {
    const response = await fetch('/api/auctions', { redirect: 'manual' })
    expect(response.status).toBe(401)
    expect(errorCode(await response.json())).toBe('UNAUTHORIZED')
  })

  step('2. accetta il login dell utente seed e restituisce un cookie di sessione', async () => {
  ...
  step('3. crea un asta con budget e slot piccoli', async () => {
  ...
  step('5. mostra i giocatori importati, tutti disponibili', async () => {
  ...
  step('6. registra un acquisto e restituisce uno stato coerente con la formula', async () => {
  ...
  step('7. rifiuta doppio acquisto, slot pieno e prezzo oltre il budget', async () => {
  ...
  step('8. toglie il giocatore comprato dal listone di default', async () => {
    expect(await availableNames()).not.toContain('Dimarco')
  })

  step('9. marca SOLD senza prezzo ne squadra, lasciando budget e slot invariati', async () => {
  ...
  step(
    '11. apre subito lo stream e ci propaga i cambiamenti',
    async () => {
   ...
   */
  step('12. su due acquisti dello stesso giocatore ne accetta uno solo', async () => {
   ...
   */
  step('13. su due acquisti diversi che sforerebbero il budget ne accetta uno solo', async () => {
...
/**
 * `server/utils/db` costruisce il pool all'import e lancia senza `DATABASE_URL`: si carica
 * dentro i test, che senza database sono saltati e non lo importano mai.
 */
async function busModules() {
  const [db, events, sse] = await Promise.all([
    import('../../server/utils/db'),
    import('../../server/utils/events'),
    import('../../server/utils/sse'),
  ])
  return {
    withTransaction: db.withTransaction,
    publishAuctionChange: events.publishAuctionChange,
    subscribeToAuction: sse.subscribeToAuction,
  }
}
 ...
 */
describe('bus di notifica delle aste', () => {
  const busAuctionId = '00000000-0000-4000-8000-0000000000ff'

  step(
    'consegna al sottoscrittore una notifica emessa in transazione',
    async () => {
      const { withTransaction, publishAuctionChange, subscribeToAuction } = await busModules()
  ...
  step(
    'non consegna a chi e sottoscritto a un altra asta',
    async () => {
      const { withTransaction, publishAuctionChange, subscribeToAuction } = await busModules()
      ...
      try {
        await withTransaction((tx) =>
          publishAuctionChange(tx, busAuctionId, { playerIds: ['p-2'] })
        )
